...
public func performRangeSearchWithPayload<Payload: Equatable>(
  ...
  try searchInNode(
    node: searchRoot, comparison: comparison, cursor: &cursor, ranges: &ranges, parentMatch: false,
    parentPayload: nil)
  ...
  return ranges.reduce([]) { mergedRanges, element in
    ...
    if last.range.upperBound == element.range.lowerBound && last.payload == element.payload {
      ...
      let newRange = NSRange(
        location: last.range.location, length: element.range.upperBound - last.range.lowerBound)
...
func searchInNode<Payload: Equatable>(
  ...
  let currentPayloadResolved =
    (currentMatch == .inherit) ? parentPayload ?? currentPayload : currentPayload
  ...
  if let node = node as? ElementNode {
    for child in node.getChildren() {
      try searchInNode(
        node: child, comparison: comparison, cursor: &cursor, ranges: &ranges,
        parentMatch: currentMatchResolved, parentPayload: currentPayloadResolved)
    }
  }
