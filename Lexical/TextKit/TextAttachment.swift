...
public class TextAttachment: NSTextAttachment {
  ...
  override public func attachmentBounds(
    for textContainer: NSTextContainer?, proposedLineFragment: CGRect, glyphPosition _: CGPoint,
    characterIndex: Int
  ) -> CGRect {
    ...
    let attributes =
      textContainer?.layoutManager?.textStorage?.attributes(at: characterIndex, effectiveRange: nil)
      ?? [:]
    ...
    try? editor.read {
      ...
      let size = decoratorNode.sizeForDecoratorView(
        textViewWidth: editor.frontend?.textLayoutWidth ?? CGFloat(0), attributes: attributes)
